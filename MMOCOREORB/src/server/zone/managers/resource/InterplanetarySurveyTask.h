...
public:
	...
	void run() {
		...
		HashTable<short, Vector<short>* > attributeMap; //added by EiF for resource attributes - original code from RogueOne?
		...
		while(familyit.hasNext()) {
			...
			for(int i = 0; i < tValues->size(); i++) {
				...
				for(int j = 0; j < values->size(); j++) {
					...
					// Added for adding attributes to mail - original code by RogueOne?
